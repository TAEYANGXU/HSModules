...
// )                 (_)_)_)  \_/`~-===-~`\_/  (_(_(_)                (
//
//        File Name:       HSWatchLiveVideoView.m
...
//加载失败
@property(nonatomic,copy) HSLiveNoDataView *smallLiveNoDataView;
//播放
...
//清晰度列表
@property(nonatomic,copy) HSDefinitionListView *definitionListView;

//**************************全屏***************************//
@property(nonatomic,copy) UIView   *bigView;
@property(nonatomic,copy) UIView   *bigTapView;
//加载失败
@property(nonatomic,copy) HSLiveNoDataView *bigLiveNoDataView;
//播放
...
//清晰度列表
@property(nonatomic,copy) HSDefinitionListView *bigDefinitionListView;
//屏幕亮度
@property(nonatomic,copy) HSLiveVideoLightView *liveVideoLightView;
...
@implementation HSWatchLiveVideoView
...
- (void)playWithVhallId:(NSNumber *)vhallId toTiew:(UIView *)view
{
    if (self.vhMoviePlayer.moviePlayerView) {
        if (self.isFullScreen) {
            [HSHUD showProgressHUDAddedToVideoView:self.bigView];
        }else {
            [HSHUD showProgressHUDAddedToVideoView:self.smallView];
        }
    }
...
-(VHallMoviePlayer *)vhMoviePlayer{
    if (_vhMoviePlayer == nil) {
        ...
        [_vhMoviePlayer setCurDefinition:VHMovieDefinitionOrigin];
    }
...
-(HSLiveNoDataView *)smallLiveNoDataView{
    if (_smallLiveNoDataView == nil) {
        _smallLiveNoDataView = [[HSLiveNoDataView alloc] initWithFrame:CGRectZero];
...
-(UIButton *)definitionButton{
    if (_definitionButton == nil) {
        ...
        [_definitionButton setTitle:@"原画" forState:UIControlStateNormal];
...
-(HSDefinitionListView *)definitionListView{
    if (_definitionListView == nil) {
        _definitionListView = [[HSDefinitionListView alloc] initWithFrame:CGRectMake(0, 0, 60, 90)];
...
-(HSDefinitionListView *)bigDefinitionListView{
    if (_bigDefinitionListView == nil) {
        _bigDefinitionListView = [[HSDefinitionListView alloc] initWithFrame:CGRectMake(0, 0, 70, 120)];
...
-(HSLiveVideoLightView *)liveVideoLightView{
    if (_liveVideoLightView == nil) {
        _liveVideoLightView = [[HSLiveVideoLightView alloc] initWithFrame:CGRectZero];
        _liveVideoLightView.hidden = YES;
    }
    return _liveVideoLightView;
}

-(HSLiveNoDataView *)bigLiveNoDataView{
    if (_bigLiveNoDataView == nil) {
        _bigLiveNoDataView = [[HSLiveNoDataView alloc] initWithFrame:CGRectZero];
